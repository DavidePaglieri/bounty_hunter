        ...
        for names in sequential:
            ...
            for sublayer_name in subset:
                if sublayer_name == "self_attn.out_proj":
                    ...
                    output_attn_o_proj = torch.cat((output_attn_o_proj, vis_handlers["self_attn.out_proj"].output.unsqueeze(0).to("cpu")), 0)
                if sublayer_name == "fc2":
                    vis_handlers[sublayer_name].normalize_output()
                    output_mlp_down_proj = torch.cat((output_mlp_down_proj, vis_handlers["fc2"].output.unsqueeze(0).to("cpu")), 0)      
...
if __name__ == "__main__":
    ...
    tokenizer_transformers = AutoTokenizer.from_pretrained(args.model_path, use_fast=True)
    ...
    for idx, language in enumerate(sorted_languages):
        ...
        torch.save(output_attn_o_proj, f"./opt_outputs/{language}.pt")
        torch.save(output_mlp_down_proj, f"./opt_outputs_fc2/{language}.pt")